<template>
  <div style="display:flex;flex-direction:column;gap:20px">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h2 style="font-size:22px;font-weight:500;color:var(--color-text-primary)">กลุ่มนักเรียน</h2>
        <p style="font-size:13px;color:var(--color-text-secondary);margin-top:3px">จัดการและแบ่งกลุ่มนักเรียนตามชั้นปีหรือเกณฑ์ที่กำหนด</p>
      </div>
      <button class="adm-hdr-btn adm-hdr-btn-primary" @click="openCreate">
        <PhPlus :size="14" /> สร้างกลุ่มนักเรียนใหม่
      </button>
    </div>
    ...
    <div class="group-grid">
      <div v-for="g in filtered" :key="g.id" class="group-card">
        ...
        <div style="margin-top:14px">
          ...
          <div class="group-meta">ID: {{ g.id }} · {{ g.memberCount }} นักเรียน</div>
        </div>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:4px;min-height:22px">
          <span
            v-for="wid in enabledPerms(g.permissions)"
            :key="wid"
            class="group-perm-badge"
          >{{ walletName(wid) }}</span>
...
function walletName(id: string) {
  return walletsStore.wallets.find(w => w.id === id)?.name ?? id
}
// กรองเฉพาะ wallet ที่ enabled เท่านั้น
...
const groups = ref<Group[]>([
  { id: 'G_K1',   name: 'กลุ่ม K1',   memberCount: 2, permissions: ['W001','W004'] },
  { id: 'G_K2',   name: 'กลุ่ม K2',   memberCount: 1, permissions: ['W001','W004'] },
  { id: 'G_P1P3', name: 'กลุ่ม P1–P3',memberCount: 3, permissions: ['W001','W002','W004'] },
  { id: 'G_P4P6', name: 'กลุ่ม P4–P6',memberCount: 3, permissions: ['W001','W002','W004'] },
  { id: 'G_SEC',  name: 'กลุ่มมัธยม', memberCount: 6, permissions: ['W001','W002','W005'] },
])
...
function openCreate()        { router.push('/admin/student-groups/new?name=สร้างกลุ่มนักเรียนใหม่') }
function openEdit(g: Group)  { router.push(`/admin/student-groups/${g.id}?name=${encodeURIComponent(g.name)}&perms=${(g.permissions??[]).join(',')}`) }
function openManage(g: Group){ router.push(`/admin/student-groups/${g.id}?name=${encodeURIComponent(g.name)}&perms=${(g.permissions??[]).join(',')}`) }
</script>
...
/* Modal (reuse global modal-backdrop) */
