<template>
  <div style="display:flex;flex-direction:column;gap:16px">
    ...
    <div>
      <h2 style="font-size:22px;font-weight:500;color:var(--color-text-primary)">ตั้งค่าประจำสัปดาห์</h2>
      <p style="font-size:13px;color:var(--color-text-secondary);margin-top:3px">กำหนดวันที่เปิดรับจอง และวันหยุดพิเศษ</p>
    </div>
    ...
    <!-- Weekly schedule -->
    <div class="bs-card">
      <div class="bs-card-title">วันที่เปิดรับจอง (ประจำสัปดาห์)</div>
    ...
    <div class="bs-card">
      ...
      >
        ...
        <div style="flex:1">
          <div style="font-size:14px;font-weight:500;color:#1C1C1E">{{ fmtDateRange(b.date, b.endDate) }}</div>
      ...
      <Transition name="modal-up">
        <div v-if="showAddBlackout" class="bs-modal">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="bs-field">
              <label class="bs-label">วันที่เริ่ม <span style="color:var(--color-danger)">*</span></label>
              <input v-model="newBlackoutDate" type="date" class="bs-input" />
            </div>
            <div class="bs-field">
              <label class="bs-label">วันที่สิ้นสุด (ไม่บังคับ)</label>
              <input v-model="newBlackoutEndDate" type="date" :min="newBlackoutDate" class="bs-input" />
            </div>
          </div>
          <div class="bs-field">
...
import { PhPlus, PhTrash, PhX, PhCalendarX } from '@phosphor-icons/vue'
import type { BookingBlackout } from '@/api/types'
import {
  getBookingConfig,
  updateBookingConfig,
...
const blackouts       = ref<BookingBlackout[]>([])
const pageError       = ref('')
const savingConfig    = ref(false)
const showAddBlackout    = ref(false)
const newBlackoutDate    = ref('')
const newBlackoutEndDate = ref('')
...
async function saveConfig() {
  ...
  try {
    const cfg = await updateBookingConfig([...openDays.value])
...
async function addBlackout() {
  ...
  blackoutError.value  = ''
  try {
    const endDate = newBlackoutEndDate.value && newBlackoutEndDate.value > newBlackoutDate.value
      ? newBlackoutEndDate.value
      : undefined
    const b = await createBookingBlackout(newBlackoutDate.value, endDate, newBlackoutReason.value || undefined)
...
function fmtDate(d: string) {
  return new Date(d + 'T00:00:00').toLocaleDateString('th-TH', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })
}
...
onMounted(async () => {
  try {
    const [cfg, bl] = await Promise.all([getBookingConfig(), listBookingBlackouts()])
    openDays.value  = cfg.openDays
    blackouts.value = bl
