<template>
  <div style="display:flex;flex-direction:column;gap:16px">
    ...
    <div>
      ...
      <p style="font-size:13px;color:var(--color-text-secondary);margin-top:3px">กำหนดวันที่เปิดบริการ และวันหยุดพิเศษ</p>
    </div>
...
import type { BuffetBlackout } from '@/api/types'
import { getBuffetConfig, updateBuffetConfig, listBuffetBlackouts, createBuffetBlackout, deleteBuffetBlackout } from '@/api/buffet'
...
async function addBlackout() {
  ...
  try {
    const endDate = newBlackoutEndDate.value && newBlackoutEndDate.value > newBlackoutDate.value
      ...
      : undefined
    const b = await createBuffetBlackout(newBlackoutDate.value, endDate, newBlackoutReason.value || undefined)
...
onMounted(async () => {
  try {
    const [cfg, bl] = await Promise.all([getBuffetConfig(), listBuffetBlackouts()])
    openDays.value  = cfg.openDays
    blackouts.value = bl
